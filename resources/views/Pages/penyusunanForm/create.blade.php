...
@section('content')
    ...
    <div class="container mt-5" style="font-size: 13px;">
        <div class="row">
            <h3 class="fw-bold">Periode Tahun {{ $year }}</h3>
            <span>DKPI ID: -</span>
        </div>
        <div class="row my-4">
                ...
                <tbody>
                    @for ($i = 0; $i < 8; $i++)
                        <tr class="text-center">
                                ...
                                <select class="form-control gradient-border py-1 w-100 indikatorKunciKerja" required>
                                    @if (count($kpis) > 0)
                                        @for ($j = 0; $j < count($kpis); $j++)
                                            <option value="{{ $kpis[$j]->iddKPI }}" data-index="{{ $j }}">{{ $kpis[$j]->indikatorKunciKerja }}</option>
                                        @endfor
                                    @else
                            ...
                            <td class="perspektif">{{ count($kpis) > 0 ? $kpis[0]->perspektif : 'Tidak ada data' }}</td>
                            <td class="tujuanStrategis">{{ count($kpis) > 0 ?  $kpis[0]->tujuanStrategis : 'Tidak ada data' }}</td>
                            <td>
                            ...
                            <td>
                                <input type="number" class="form-control gradient-border py-1 w-100 bobot" placeholder="..." required>
                            </td>
                            <td>
                                <input type="number" class="form-control gradient-border py-1 w-100 targetTriwulan1" placeholder="..." required>
                            </td>
                            <td>
                                <input type="number" class="form-control gradient-border py-1 w-100 targetTriwulan2" placeholder="..." required>
                            </td>
                            <td>
                                <input type="number" class="form-control gradient-border py-1 w-100 targetTriwulan3" placeholder="..." required>
                            </td>
                            <td>
                                <input type="number" class="form-control gradient-border py-1 w-100 targetTriwulan4" placeholder="..." required>
                            </td>
                        </tr>
                    @endfor
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="5" class="text-end">
                            <span class="fw-bold">
                                Total Bobot: <span id="total_bobot" class="text-danger">{{ $kpis->sum('bobot') }}</span>
                            </span>
                        </td>
                    </tr>
                </tfoot>
...
@section('script')
    <script>
        ...
        if(!kpis.length) {
            $('#btn_save').attr('disabled', true).text('Harap tambahkan Indikator Kunci Kerja terlebih dahulu!');
        }
        ...
        $('#btn_save').click(function() {
            $(this).attr('disabled', true).text('Menyimpan...');
            ...
            $.ajax({
                url: "{{ route('penyusunan-kpi.store') }}",
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    data: data
                ...
                error: function(err) {
                    $('#btn_save').removeClass('btn-primary').addClass('btn-danger').text('Gagal menyimpan..');
                    console.log(err);
                }
            })
        })
